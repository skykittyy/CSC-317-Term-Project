...
db.serialize(() => {
  ...
  `, (err) => {
    ...
    // Seed plushies if table is empty
    db.get('SELECT COUNT(*) AS count FROM products', (err, row) => {
      ...
      if (row.count === 0) {
        const products = [
          // ... your products array here (unchanged)
          {
...
Whether he's chilling on your bed, riding in your backpack, or starring in an imaginary barnyard adventure, Cluckleberry Puff is your go-to pal for comfort, giggles, and good vibes all day long!`,
          ...
          {
            name: '🐮Moo Moo the Mini Cow🍼',
            price: 19.99,
            description: `Say hello to Moo Moo, the snuggliest little calf on the farm! 
With creamy white fur, soft caramel patches, and the sweetest pink snout, Moo Moo is made for hugs. 
Her tiny horns and floppy ears add extra charm, while her big black eyes are full of love and curiosity. 
Perfect for bedtime cuddles, desk decoration, or a travel buddy in your bag, Moo Moo brings comfort, smiles, and a touch of farmyard magic wherever she goes!`,
            image: 'https://auroragift.com/cdn/shop/files/82172.jpg?v=1746128232' // Replace with actual image URL
          },
          {
            name: '🦄Twinklehoof Sparklesnug🌙',
            price: 22.50,
            description: `Introducing Twinklehoof Sparklesnug — your dreamy sidekick from the land of fluff and stardust! 
With her velvety white fur, rosy pink hooves, and a candy-colored horn that glistens with giggles, 
Twinklehoof is pure enchantment in plushie form. Her soft pastel wings are made for magical journeys, 
and her sleepy embroidered eyes promise sweet dreams every night. 
Whether she’s perched on your pillow, riding rainbows, or just being adorably fabulous, 
Twinklehoof is the cuddly unicorn you've always wished for!`,
            image: 'https://omilion.com/cdn/shop/products/61agJ3jF9jL_1024x1024.jpg?v=1619848805' // Replace with actual image URL
          },
          {
            name: '🐱Mochi Meowchi the Blush Kitty🍡',
            price: 18.75,
            description: `Say hi to Mochi Meowchi — the purr-fectly squishy cube of cuteness! 
With her pastel pink plush, chubby cheeks, and tiny embroidered whiskers, she's as soft as a marshmallow and twice as sweet. 
Her round little paws and kawaii expression make her the ultimate snuggle buddy, desk companion, or mood booster. 
Whether you're lounging, gaming, or just need a pocket-sized dose of joy, Mochi Meowchi is here to make life extra cuddly and cute!`,
            image: 'https://www.stuffedsafari.com/v/vspfiles/photos/AR-14273-2.jpg?v-cache=1740511032' // Replace with actual image URL
          }
        ];

        const stmt = db.prepare('INSERT INTO products (name, price, description, image) VALUES (?, ?, ?, ?)');
        products.forEach(p => {
          stmt.run(p.name, p.price, p.description, p.image);
          // Removed callback in stmt.run to avoid multiple console errors
        });
        stmt.finalize(() => {
          console.log('Plushies seeded to products table.');
        });
      }
    });
  });

  // Create the cart_items table if it doesn't exist (new)
  db.run(`
